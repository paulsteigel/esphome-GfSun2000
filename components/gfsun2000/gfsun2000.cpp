...
static const char *const TAG = "gfsun2000";
...
void gfsun2000::on_modbus_data(const std::vector<uint8_t> &data) {
  ...
  ESP_LOGW(TAG, "Invalid size (%zu) for gfsun2000 frame!", data.size());
  ESP_LOGW(TAG, "Payload: %s", format_hex_pretty(&data.front(), data.size()).c_str());
}

void gfsun2000::on_acknowledge_data_(const std::vector<uint8_t> &data) {
  auto gfsun2000_get_16bit = [&](size_t i) -> uint16_t {
  ...
  uint16_t starting_address = gfsun2000_get_16bit(0);
  uint16_t registers_written = gfsun2000_get_16bit(2);
...
void gfsun2000::on_status_data_(const std::vector<uint8_t> &data) {
  auto gfsun2000_get_16bit = [&](size_t i) -> uint16_t {
    return (uint16_t(data[i + 0]) << 8) | (uint16_t(data[i + 1]) << 0);
  };
  ...
  uint16_t model_number = gfsun2000_get_16bit(22);
  switch (model_number) {
    case 5015:
    case 5020:
      this->set_current_resolution_if_auto(gfsun2000_CURRENT_RESOLUTION_LOW);
      this->publish_state_(this->device_model_text_sensor_, "gfsun2000" + to_string(model_number));
      break;
    case 5205:
      this->set_current_resolution_if_auto(gfsun2000_CURRENT_RESOLUTION_HIGH);
  ...
  float voltage_setting = gfsun2000_get_16bit(0) * 0.01f;
  ...
  float current_setting = gfsun2000_get_16bit(2) * this->current_resolution_factor();
  ...
  this->publish_state_(this->output_voltage_sensor_, (float) gfsun2000_get_16bit(4) * 0.01f);
  //   6    0x00 0xED        Output current display value     0237 * 0.01 = 2.37A           0.01 A
  this->publish_state_(this->output_current_sensor_, (float) gfsun2000_get_16bit(6) * this->current_resolution_factor());
  //   8    0x21 0x4F        Output power display value       8527 * 0.01 = 85.27W          0.01 W
  this->publish_state_(this->output_power_sensor_, (float) gfsun2000_get_16bit(8) * 0.01f);
  //  10    0x10 0x87        Input voltage display value      4231 * 0.01 = 42.31V          0.01 V
  this->publish_state_(this->input_voltage_sensor_, (float) gfsun2000_get_16bit(10) * 0.01f);
  //  12    0x00 0x00        Key lock                         0x00: off, 0x01: on
  bool key_lock = gfsun2000_get_16bit(12) == 0x0001;
  ...
  this->publish_state_(this->constant_current_mode_binary_sensor_, gfsun2000_get_16bit(16) == 0x0001);
  //  18    0x00 0x01        Switch output state              0x00: off, 0x01: on
  bool output = gfsun2000_get_16bit(18) == 0x0001;
  ...
  this->publish_state_(this->backlight_brightness_sensor_, gfsun2000_get_16bit(20) * 20.0f);
  //  22    0x13 0x9C        Product model                    5020 = gfsun20005020
  //  24    0x00 0x11        Firmware version                 17 * 0.1 = 1.7
  this->publish_state_(this->firmware_version_sensor_, gfsun2000_get_16bit(24) * 0.1f);
}
...
void gfsun2000::write_register(uint16_t address, uint16_t value) {
...
void gfsun2000::publish_state_(binary_sensor::BinarySensor *binary_sensor, const bool &state) {
...
void gfsun2000::publish_state_(number::Number *number, float value) {
  if (number == nullptr)
    return;
...
void gfsun2000::publish_state_(sensor::Sensor *sensor, float value) {
  if (sensor == nullptr)
    return;
...
void gfsun2000::publish_state_(text_sensor::TextSensor *text_sensor, const std::string &state) {
...
void gfsun2000::publish_state_(switch_::Switch *obj, const bool &state) {
  if (obj == nullptr)
    return;
...
void gfsun2000::dump_config() {  // NOLINT(google-readability-function-size,readability-function-size)
