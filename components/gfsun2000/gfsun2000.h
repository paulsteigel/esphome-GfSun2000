...
enum CurrentResolution {
  fsun2000_CURRENT_RESOLUTION_AUTO,
  fsun2000_CURRENT_RESOLUTION_LOW,
  fsun2000_CURRENT_RESOLUTION_HIGH,
};

class fsun2000 : public PollingComponent, public modbus::ModbusDevice {
 public:
  ...
  void set_current_resolution_if_auto(CurrentResolution current_resolution) {
    if (this->current_resolution_ == fsun2000_CURRENT_RESOLUTION_AUTO) {
      this->set_current_resolution(current_resolution);
    }
  }
  float current_resolution_factor() {
    return (this->current_resolution_ == fsun2000_CURRENT_RESOLUTION_HIGH) ? 0.001f : 0.01f;
  }
 ...
 protected:
  CurrentResolution current_resolution_{fsun2000_CURRENT_RESOLUTION_AUTO};
